...
```js
const fullName = (first, second) => console.log(`${first} ${second}`);
```
...
3. Crazy number again!!
   ...
   //
   // Write your solution here
   stringNumbers.forEach(n => {
     if (Number(n) < 4000) {
       totalNumbersUnder4000 += Number(n);
     }
   });
   ```
...
3. Abbreviations
   ...
   // Write your solution here
   dayAbbreviations = days.map(word => word.slice(0, 3));
   ```
...
3. century20
   ...
   // century20 should be: [1989, 2000, 1999, 1973]
   // Write your solution here
   century20 = years.filter(y => y < 2001);
   ```
...
3. Crazy number again!!
   ...
   //
   // Write your solution here
   totalNumbersUnder4000 = stringNumbers.reduce((f, s) => {
     if (Number(s) < 4000) {
...
// Write your solution here
let cheapProducts = products.filter(a => {
  return a.price < 70;
});
