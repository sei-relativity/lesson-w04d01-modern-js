...
2. Capitalize
   ...
   const iAS = ['alanoud', 'reem', 'hazim', 'mansour'];
let capitalizedIAs = iAS.map(name => name.toUpperCase())
...
2. Create an array of names (maybe use 3 of your friends)
    ...
    const names = ["Lamees", "Nawal", "Mesfer"];
let result = names.filter(name=>name.includes('a'));
    console.log(result);
    ```
3. century20
    ```js
...
let newString = [];
  stringNumbers.forEach(crazyNumber => {
   ...
   totalNumbersUnder4000 = newString.reduce( (acc, val) => acc + (val< 4000));
   console.log(totalNumbersUnder4000);
    ```
...
let discountProduct1 =products.map(pro =>( pro.price= pro.price/2 
return pro});
...
let cheapProducts = products.filter(pro => pro.price < 70 );
