...
century20 = years.map(item => { if (item <= 2000 && item >= 1900) 
return item; 
});
    ```
...
1. Only get the numbers that are divisible by 3
    ```js
const divisibleBy = nums.filter(item => {
    if (item % 3 === 0) {
        return item;
    }
});
...
2. Create an array of names (maybe use 3 of your friends)
 ...
 const nameA = names.filter(item => item.includes('a'));
    ```js

    ```
3. century20
    ```js
	  century20 = years.filter(item => {
    if(item>= 1900 && item<= 2000){
      return item;   
    }
});
    ```
...
const nums = [103440, 3799.2663, 3.14159265359, 859494, 59439];
let total = nums.reduce((sum,item) => sum + item);
console.log(total);
    ```
3. Crazy number again!!
    ```js
